class PlayerCardComponent extends HTMLElement {
    static attributesMap = {
        state: 'updateState', //text: 'updateText',
        character: 'character', username: 'username', transition: 'updateTransition',
    };
    ...
    static get observedAttributes() {
        return Object.keys(PlayerCardComponent.attributesMap);
    }

    attributeChangedCallback(name, oldValue, newValue) {
        const method = PlayerCardComponent.attributesMap[name];
    ...
    connectedCallback() {
        Object.keys(PlayerCardComponent.attributesMap).forEach((attr) => {
            if (this.hasAttribute(attr)) {
                const method = PlayerCardComponent.attributesMap[attr];
                this[method](this.getAttribute(attr));
            }
        })
    }
    ...
    updateState(newState) {
        ...
        if (stateIndicator === '_1') {
            if (!element.classList.contains(stateName)) {
                element.classList.add(stateName);
            }
        } // stateName does not already exist, execute addition
        if (stateIndicator === '_0') {
            if (element.classList.contains(stateName)) {
                element.classList.remove(stateName);
            }
        } // stateName exists, execute removal
    }
